...
async function readFilePract(){
 ...
 export async function createPract(data){
   ...
   let practices = [...practicesExist, newPractices]
...
export async function deletePract(id) {
    const practices = await read()
   let delPract = practices.find( p => p.id == +id)
   if(!delPract){
    console.log('No such practic')
    return
   }
  let newPractices =  practices.filter(pract => pract.id !== +id)
...
export async function updatePr(objData) {
    if(!objData){return}
    const practices = await read()
    let updatePract = practices.find( p => p.id == +objData.id)
